...
def makeMove(board, chests, x, y):
    ...
    if smallestDistance == 0:
        ...
        chests.remove([x, y])
        return 'Вы нашли сундук на затонувшем судне!'
    else:
        if smallestDistance < 10:
            board[x][y] = str(smallestDistance)
            return 'Сундук с сокровищами обнаружен на роасстоянии %s от гидролокатора.' % (smallestDistance)
        else:
             board[x][y] = 'X'
    ...
    while True:
        ...
        if len(move) == 2 and move[0].isdigit() and move[1].isdigit() and isOnBoard(int(move[0]), int(move[1])):
            if [int(move[0]), int(move[1])] in previousMoves:
                print('Вы уже опускали гидролокатор здесь')
                continue
...
def showInstructions():
    print('''Вы - капитан корабля плывущего за сокровищами. Ваша задача - с помощью
    	  гидролокаторов найти три сундука с скоровищами в затонувших суднах на дне океана.
    	  ...
    	  число, обозначающее, на каком расстоянии ближайший сундук. Или будет показана 
    	  буква Х, обозначающая, что сундук в области действия гидролокатора не обнаружен.
    	  На карте ниже отметки С - это сундуки.
...
Нажмите клавишу Enter, чтобы продолжить...''')
    ...
    print(''' Если гидролокатор опущен прямо на сундук, вы сможете поднять сундук. 
    	Другие локаторы обновят данные о расположении ближайшего сундука.
...
print('С О Н А Р !') 
print()
print('Показать обучение? (да или нет)')
...
while True:
    ...
    while sonarDevices > 0:
        ...
        print('Осталось гидролакторов: %s. Осталось сундуков с сокровищами: %s.' % (sonarDevices, len(theChests)))
